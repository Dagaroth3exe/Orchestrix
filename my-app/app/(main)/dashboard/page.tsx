...
import { BookOpen, CheckSquare, Bell, Clock, User, ArrowRight, LogOut, Moon, Sun } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const iconButtonClass = "group relative h-9 w-9 overflow-hidden rounded-xl border-[5px] border-zinc-200 bg-zinc-300 text-zinc-950 shadow-[0_8px_18px_rgba(0,0,0,0.18)] transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] hover:w-20 hover:border-zinc-300 hover:bg-zinc-200 hover:shadow-[0_12px_24px_rgba(0,0,0,0.22)] hover:-translate-y-0.5";

  return (
    <div className="w-full h-full">
      <div className="Main bg-amber-50/80 dark:bg-zinc-900 h-full w-full rounded-2xl p-3 flex flex-col md:flex-row gap-3 transition-colors duration-200">
        ...
        <div className="hidden md:flex h-full w-[7%] flex-col items-center justify-start pt-2 px-2">
          <button type="button" onClick={() => router.push("/")} aria-label="Go to intro screen" className="cursor-pointer">
            <Image src={ShortBlackLogo} alt="Logo" width={72} height={80} className="object-contain dark:invert" />
          </button>
          <div className="mt-8 flex flex-col gap-4 items-center">
            ...
            >
              <span className="absolute inset-0 grid place-items-center transition-all duration-300 ease-out group-hover:opacity-0 group-hover:scale-90">
                {dark ? <Sun className="size-5" style={{ color: "#231E1F" }} /> : <Moon className="size-5" style={{ color: "#231E1F" }} />}
              </span>
              <span className="absolute inset-0 flex items-center justify-center text-[14px] font-semibold tracking-[0.04em] [font-family:var(--font-outfit)] opacity-0 transition-all duration-300 ease-out group-hover:opacity-100 group-hover:scale-100">
                {dark ? "Light" : "Dark"}
              </span>
          ...
          <div ref={profileRef} className="relative mt-auto mb-2">
            {profileOpen && (
              <div className="absolute bottom-full left-0 mb-3 w-56 bg-[#231E1F] dark:bg-zinc-700 rounded-2xl shadow-2xl overflow-hidden z-50">
        ...
        <div className="flex-1 flex flex-col min-h-0 gap-3">
          ...
          <div className="flex md:hidden items-center justify-between px-1 pt-1">
            <button onClick={() => router.push("/")} aria-label="Home">
              <Image src={ShortBlackLogo} alt="Logo" width={48} height={54} className="object-contain dark:invert" />
            </button>
            <div className="flex items-center gap-2">
              <button
                ...
                aria-label="Toggle dark mode"
              >
                {dark ? <Sun className="size-4" style={{ color: "#231E1F" }} /> : <Moon className="size-4" style={{ color: "#231E1F" }} />}
              </button>
              <div className="relative">
                ...
                {profileOpen && (
                  <div className="absolute top-full right-0 mt-2 w-56 bg-[#231E1F] dark:bg-zinc-700 rounded-2xl shadow-2xl overflow-hidden z-50">
                    <div className="p-4 border-b border-white/10">
                      <p className="text-white font-semibold text-sm [font-family:var(--font-outfit)] truncate">
                        {userName || "User"}
                      </p>
          ...
          {/* Dashboard content */}
          <div className="InfoBox flex flex-col gap-4 bg-amber-50 dark:bg-zinc-800 flex-1 min-h-0 w-full rounded-2xl p-4 md:p-6 md:pl-8 overflow-y-auto transition-colors duration-200">
            {/* Recent Notes */}
            <div>
              <div className="flex items-center justify-between mb-4">
                <div className="text-2xl md:text-4xl text-[#231E1F] dark:text-zinc-100 [font-family:var(--font-outfit)] font-semibold">
                  Recent Notes
                </div>
                <button
                  onClick={() => router.push("/Notes")}
                  className="flex items-center gap-2 text-[#231E1F] dark:text-zinc-300 hover:opacity-70 transition-opacity [font-family:var(--font-outfit)] font-semibold text-sm"
                >
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-2">
                {recentNotes.length === 0 ? (
                  <p className="text-[#231E1F]/40 dark:text-zinc-500 text-sm [font-family:var(--font-outfit)] py-2">
          ...
          {/* Mobile bottom nav */}
          <div className="flex md:hidden items-center justify-around bg-amber-50 dark:bg-zinc-900 rounded-2xl py-3 px-4 transition-colors duration-200">
            {navButtons.map(({ icon: Icon, label }) => (
              <button
                key={label}
                onClick={() => router.push(`/${label}`)}
                aria-label={label}
                className="flex flex-col items-center gap-1 text-[#231E1F] dark:text-zinc-300 hover:opacity-70 transition-opacity"
              >
            ...
            >
              ...
              <span className="text-[10px] font-semibold [font-family:var(--font-outfit)]">{dark ? "Light" : "Dark"}</span>
            </button>
          </div>
        </div>
      </div>
